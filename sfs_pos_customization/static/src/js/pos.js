openerp.sfs_pos_customization = function(db) {
    ...
    var Pos = Backbone.Model.extend({
        ...
        get_app_data: function() {
            ...
            return $.when(new db.web.Model("sale.shop").get_func("search_read")([]).pipe(function(result) {
                ...
                return new db.web.Model("res.company").get_func("read")(company_id, ['currency_id', 'name', 'phone', 'logo']).pipe(function(result) {
                    ...
                    return new db.web.Model("res.currency").get_func("read")([currency_id],
                            ['symbol', 'position', 'name']).pipe(function(result) {
    ...
    var Order = Backbone.Model.extend({
        ...
        setChangeAmount: function(){
            ...
            comp_curr_rate = 0.00
            payment_line = this.get('paymentLines')
            if (payment_line){
                payment_line = payment_line.first()
                comp_curr_rate = payment_line.get('comp_curr_rate')
            }
            remainingAmount = dueTotal - paidTotal;
            remaining = remainingAmount > 0 ? 0 : (-remainingAmount).toFixed(2);
            $('#payment-remaining').html((remaining * comp_curr_rate).toFixed(2));
...
//        },
        ...
        validate:function(){
            ...
            orders = this.shop.get('orders').first()
            orders.get('paymentLines').each(function(item){
                amount = item.get('amount')
                total += amount;
            });
            paymentdue = parseFloat($("#net-total").text());
        ...
        render_element: function() {
        	...
        	var journal_currency = this.model.get('currency')[1]
        	var currency_name = 'MXN ($)'
        	if (journal_currency == 'USD ($)'){
        	    var currency_name = 'MXN ($)'
        	};
        	if (journal_currency == 'MXN ($)'){
            ...
            this.$element.html(this.template_fct({
                ...
                conv_amount: (this.amount * this.model.get('comp_curr_rate')) / this.model.get('curr_rate'),
                conv_name: "Cantidad en " + currency_name
            }));
            this.$element.addClass('paymentline');
            $('input', this.$element).change(_.bind(this.changeAmount, this));
    ...
    var PaymentWidget = db.web.OldWidget.extend({
        ...
        back: function() {
            ...
            currentOrder = this.shop.get('selectedOrder');
            console.log(currentOrder.get('paymentLines').models)
            currentOrder.get('paymentLines').models = []
        ...
        updatePaymentSummary: function(cashRegister) {
            ...
            comp_curr_rate = 0.00
            currentPaymentLines = (this.shop.get('selectedOrder')).get('paymentLines');
            if (currentPaymentLines){
                paymentline = currentPaymentLines.first()
                if (paymentline){
                    comp_curr_rate = paymentline.get('comp_curr_rate')
                }
            }
            $("#net-total").html(netTotal.toFixed(2));
            this.$element.find('#payment-due-total').html(dueTotal.toFixed(2));
            this.$element.find('#payment-paid-total').html((paidTotal * comp_curr_rate).toFixed(2));
            
            remainingAmount = dueTotal - paidTotal;
            remaining = remainingAmount > 0 ? 0.00 : (-remainingAmount).toFixed(2);
            if (isNaN(remaining)){
                remaining = 0
            }
            $('#payment-remaining').html((remaining * comp_curr_rate).toFixed(2));
            $('#payment-remaining-usd').html(remaining);
            $('#payment-remaining_last').html(remaining);
        },
        setNumpadState: function(numpadState) {
        	if (this.numpadState) {
    ...
    var ReceiptWidget = db.web.OldWidget.extend({
        ...
        finishOrder: function() {
            ...
            salesman_ele.value = 'Select'
            var partner_ele = document.getElementById('partner');
            partner_ele.value = 'Select'
        },
        changeSelectedOrder: function() {
    ...
    db.point_of_sale.PointOfSale = db.web.OldWidget.extend({
        ...
        render: function() {
            ...
            $('.pos-step-container').replaceWith(QWeb.render('payment_screen_inherit'))
        },
