...
class point_of_sale(osv.osv):
    ...
    def convert(self, cr, uid, ids, amount, company_id, context=None):
        ...
        context={}
        amount = float(amount)
        usd_ids = curr_pool.search(cr, uid, [('name', '=', 'USD')], context=context)
        if usd_ids:
            currency = curr_pool.browse(cr, uid, usd_ids[0], context=context)
            currency_name = 'DOLARES'
            amt = amount_to_text_sp.get_amount_to_text(self, amount, 'es_cheque', currency_name, currency.sub_currency)
        return amt
    ...
    _columns = {
                ...
                'amount_paid': fields.function(_amount_all, string='Paid', states={'draft': [('readonly', False)]},
                                               readonly=True, multi='all', digits_compute=dp.get_precision('Point Of Sale')),
    ...
    def action_invoice(self, cr, uid, ids, context=None):
        ...
        for pos_obj in self.browse(cr, uid, ids, context=context):
            ...
            if invoice_id:
                ...
                inv_line_vals = {'account_analytic_id': analytic_account_id}
                if pos_obj.discount_amount > 0.00:
                    inv_line_vals['discount'] = pos_obj.discount_percentage
                invoice_line_pool.write(cr, uid, invoice_line_ids, inv_line_vals, context=context)
#                if pos_obj.discount_amount > 0.00:
